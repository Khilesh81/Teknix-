...
const Carousel = () => {
  const { data, fetchAllProducts } = getData(DataContext);
  // console.log(data);
            ...
            >
              <div className="flex flex-col md:flex-row gap-10 justify-center h-[600px] my-20 md:my-0 items-center px-4">
                <div className="md:space-y-6 space-y-3">
                  <h3 className="text-red-500 font-semibold font-serif text-sm">
                    Powering Your World with the best in Electronics
                  </h3>
                  <h1 className="text-xl md:text-4xl font-bold uppercase line-clamp-2 md:line-clamp-3 text-white md:w-[500px]">
                    {item.title}
                  </h1>
                  <p className="md:w-[500px] text-gray-400 mr-7 line-clamp-3">
                    {item.description}
                  </p>
